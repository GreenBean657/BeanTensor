...
void test_make_tensor_sets_metadata_and_strides() {
    ...
    Tensor t = BeanTensor::Tensors::make_tensor(shape, 3, DType::Float32, Tensor::Device::CPU, false);

    check(t.owns_data, "make_tensor should mark allocated tensors as owning their memory");
    check(t.ndim == 3, "ndim should match input rank");
    check(t.shape[0] == 2 && t.shape[1] == 3 && t.shape[2] == 4, "shape should be copied");
    check(t.strides[0] == 12 && t.strides[1] == 4 && t.strides[2] == 1, "strides should be row-major");
    check(t.numel == 24, "numel should be product of shape");
    check(t.nbytes == 24 * sizeof(float), "nbytes should match dtype size times numel");
...
void test_at_location_maps_multi_index_to_flat_memory() {
    ...
    Tensor t = BeanTensor::Tensors::make_tensor(shape, 2, DType::Int32, Tensor::Device::CPU, false);
    std::memset(t.data, 0, t.nbytes);

    auto* at_12 = BeanTensor::Tensors::at_location<int32_t>(&t, {1, 2});
    *at_12 = 42;

    auto* flat = static_cast<int32_t*>(t.data);
...
void test_transpose_returns_non_contiguous_view() {
    const size_t shape[] = {2, 3};
    Tensor t = BeanTensor::Tensors::make_tensor(shape, 2, DType::Float32, Tensor::Device::CPU, false);

    Tensor tr = BeanTensor::Tensors::transpose(t, 0, 1);

    check(tr.data == t.data, "transpose should return a view sharing the same data pointer");
    check(!tr.owns_data, "transpose view must not own parent memory");
    check(!tr.is_contiguous, "transpose view should be marked non-contiguous");
    check(tr.shape[0] == 3 && tr.shape[1] == 2, "transpose should swap selected shape dimensions");
    check(tr.strides[0] == 1 && tr.strides[1] == 3, "transpose should swap selected stride dimensions");
...
void test_deep_copy_allocates_independent_storage() {
    ...
    Tensor src = BeanTensor::Tensors::make_tensor(shape, 2, DType::Int32, Tensor::Device::CPU, false);

    auto* src_data = static_cast<int32_t*>(src.data);
    src_data[0] = 1;
    src_data[1] = 2;
    src_data[2] = 3;
    src_data[3] = 4;
    ...
    auto* copy_data = static_cast<int32_t*>(copy.data);

    check(copy.owns_data, "deep_copy should produce an owning tensor");
...
void test_shape_and_device_helpers() {
    ...
    Tensor a = BeanTensor::Tensors::make_tensor(shape_a, 2, DType::Float32, Tensor::Device::CPU, false);
    Tensor b = BeanTensor::Tensors::make_tensor(shape_b, 2, DType::Float32, Tensor::Device::CPU, false);
    Tensor c = BeanTensor::Tensors::make_tensor(shape_c, 2, DType::Float32, Tensor::Device::CPU, false);
